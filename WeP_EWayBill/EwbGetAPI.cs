...
namespace WeP_EWayBill
{
    public static class EwbGetApi
    {
            ...
            string SessionCustId, string SessionUsername, out string OStatus, out string OResponse, out string EWBDate)
        {
            ...
            try
            {
                ...
                if (status_response == "1")
                {
                    ...
                    string DecryptjsonData = EncryptionUtils.DecryptBySymmerticKey(data_response, Convert.FromBase64String(rek));
                    DecryptjsonData = Helper.Base64Decode(DecryptjsonData);
                    var finaldata = JsonConvert.DeserializeObject<List<EwbResponseAttributes>>(DecryptjsonData);
                    ...
                    if (finaldata != null)
                    {
                        ...
                        {
                            EwbBusinessLayer.Insert_Update_Otherpary(data.ewbNo, data.ewayBillDate, data.genMode, data.genGstin, data.docNo, data.docDate, data.fromGstin,
            ...
            try
            {
                ...
                if (status_response == "1")
                {
                    ...
                    if (finaldata != null)
                    {
                        foreach (var data in finaldata)
                        {
                            EwbBusinessLayer.Insert_Update_TransPorter_GETAPI(data.ewbNo, data.ewbDate, data.genGstin, GSTINNo, data.docNo, data.docDate,
            ...
            try
            {
                ...
                if (status_response == "1")
                {
                    ...
                    if (finaldata != null)
                    {
                        foreach (var data in finaldata)
                        {
                            EwbBusinessLayer.Insert_Update_TransPorter_GETAPI(GSTINNo, data.ewbNo, data.ewbDate, data.genGstin, data.docNo, data.docDate,
                              Convert.ToInt32(data.delPinCode), Convert.ToInt32(data.delStateCode), data.delPlace, data.validUpto, data.extendedTimes,
                    ...
                    if (finaldata != null)
                    {
                        ...
                        if (rowcount != 1)
                        {
                            using (SqlCommand cmd = new SqlCommand())
                            {
                                ...
                                consewbid = Convert.ToInt32(SQLHelper.InsertIntoTable("TBL_EWB_GEN_CONSOLIDATED", cmd, Con));
                                Con.Close();
        ...
        public static string GET_ErrorList(string GSTINNo)
        {
            ...
            string URL = "";
            EwbResponseAttributes EWB_Data = null;
            string strErrorCode = "", strErrorDesc = "";
            try
            {
                Helper.EWB_DataAdapter(GSTINNo, out Username, out AppKey, out EncryptedSEK, out AuthToken);
