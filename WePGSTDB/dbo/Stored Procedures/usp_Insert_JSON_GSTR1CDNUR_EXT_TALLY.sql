...
Description : Procedure to Insert the GSTR1 CDNUR JSON Records to the corresponding external tables
...
exec usp_Insert_JSON_GSTR1CDNUR_EXT  'POS',
 */

CREATE PROCEDURE [usp_Insert_JSON_GSTR1CDNUR_EXT_TALLY]
...
Begin
	...
	Select @TotalRecordsCount = count(*) from #TBL_EXT_GSTR1_CDNUR
	...
	Begin
		...
		Update #TBL_EXT_GSTR1_CDNUR -- Introduced due to Excel Format Issue
		...
		Update #TBL_EXT_GSTR1_CDNUR 
		Set ErrorCode = -13,
			ErrorMessage = dbo.udf_ValidateGstAmount('CDNUR',Gstin,'','','',Rt,Txval,Iamt,CAmt,Samt,Csamt,'','','',Inum,Idt,P_Gst) 
	...
	Begin
		...
		nt_dt as [Note/Refund Voucher date],	
		ntty as [Document Type],	
		inum as [Invoice/Advance Receipt Number],	
		idt as [Invoice/Advance Receipt date],	
		rsn as [Reason For Issuing document],	
		'' as [Place Of Supply],	
		...
		ErrorCode,
		ErrorMessage
		From #TBL_EXT_GSTR1_CDNUR	Where IsNull(ErrorCode,0) <> 0
	End
	Else
	Begin
		Select 
