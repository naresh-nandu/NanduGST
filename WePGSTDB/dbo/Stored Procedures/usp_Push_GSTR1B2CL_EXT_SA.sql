...
Description : Procedure to Insert the External GSTR1 B2CL Records to the corresponding Staging
				Area tables
...
exec usp_Push_GSTR1B2CL_EXT_SA  'POS','POSDEV1', '27AHQPA7588L1ZJ'

 */

CREATE PROCEDURE [usp_Push_GSTR1B2CL_EXT_SA]
...
Begin
	...
	(
		...
		Group By gstin,fp,gt,cur_gt,pos,inum,idt,val,etin,rt
	)t1
	Cross Apply 
	(
	...
	SET #TBL_EXT_GSTR1_B2CL_INV.gstr1id = t2.gstr1id 
	FROM #TBL_EXT_GSTR1_B2CL_INV t1,
			TBL_GSTR1 t2 
	...
	-- Insert Records into Table TBL_GSTR1_B2CL

	Insert TBL_GSTR1_B2CL(gstr1id,pos,gstinid) 
	Select	distinct gstr1Id,pos,gstinid 
	From #TBL_EXT_GSTR1_B2CL_INV t1
	Where Not Exists ( SELECT 1 FROM TBL_GSTR1_B2CL t2 where t2.gstr1id = t1.gstr1id and t2.pos = t1.pos) 

	Update #TBL_EXT_GSTR1_B2CL_INV 
	SET #TBL_EXT_GSTR1_B2CL_INV.b2clid = t2.b2clid 
	FROM #TBL_EXT_GSTR1_B2CL_INV t1,
			TBL_GSTR1_B2CL t2 
	WHERE t1.gstr1id = t2.gstr1id 
	And t1.pos = t2.pos 

	-- Insert Records into Table TBL_GSTR1_B2CL_INV

	Insert TBL_GSTR1_B2CL_INV
	(b2clid,inum,idt,val,etin,gstinid,gstr1id,custid,createdby,createddate)
	Select	distinct t1.b2clid,t1.inum,t1.idt,t1.val,t1.etin,t1.gstinid,t1.gstr1id,
					@Custid,@CreatedBy,GetDate()
	From #TBL_EXT_GSTR1_B2CL_INV t1
	Where	Not Exists ( SELECT 1 FROM TBL_GSTR1_B2CL_INV t2
	...
	Update #TBL_EXT_GSTR1_B2CL_INV 
	SET #TBL_EXT_GSTR1_B2CL_INV.invid = t2.invid 
	FROM #TBL_EXT_GSTR1_B2CL_INV t1,
	...
	Update #TBL_EXT_GSTR1_B2CL_INV 
	SET #TBL_EXT_GSTR1_B2CL_INV.num = t3.num
	FROM #TBL_EXT_GSTR1_B2CL_INV t1,
			(Select slno,invid,Row_Number()  OVER(Partition By t2.invid Order By t2.invid) as num
			FROM #TBL_EXT_GSTR1_B2CL_INV t2
	   		) t3
	Where t1.invid = t3.invid
	And t1.slno = t3.slno

	Insert TBL_GSTR1_B2CL_INV_ITMS
	(invid,num,gstinid,gstr1id)
	Select	distinct t1.invid,t1.num,t1.gstinid,t1.gstr1id
	From #TBL_EXT_GSTR1_B2CL_INV t1
	Where Not Exists ( SELECT 1 FROM TBL_GSTR1_B2CL_INV_ITMS t2
						Where t2.invid = t1.invid 
						And t2.num = t1.num)
		
	
	Update #TBL_EXT_GSTR1_B2CL_INV 
	SET #TBL_EXT_GSTR1_B2CL_INV.itmsid = t2.itmsid
	FROM #TBL_EXT_GSTR1_B2CL_INV t1,
			TBL_GSTR1_B2CL_INV_ITMS t2 
	...
	Insert TBL_GSTR1_B2CL_INV_ITMS_DET
