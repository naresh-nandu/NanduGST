...
exec usp_Insert_JSON_Mobile_App_Purchases  '33GSPTN0802G1ZL','WEP001','{
...
CREATE PROCEDURE [dbo].[usp_Insert_JSON_Mobile_App_Purchases]
...
Begin
	...
	BEGIN	
		...
		Select	space(50) as PurchasesId,
				space(50) as PurchasesSubId,
				@Gstin as Gstin,
				...
				Into #TBL_MOBILE_APP_PURCHASES
		From OPENJSON(@RecordContents) 	
		WITH
		(
			PurchasesData nvarchar(max) as JSON
		) as Purchases
		Cross Apply OPENJSON(Purchases.PurchasesData)
		WITH
		(		
		...
		Select @TotalRecordsCount = count(*) from #TBL_MOBILE_APP_PURCHASES

			

		Begin Try
			Begin
				Insert TBL_MOBILE_APP_PURCHASES	(Gstin, ReferenceNo, InvoiceDate, createddate, MobileAppId) 
					Select Distinct Gstin, ReferenceNo, CONVERT(VARCHAR(50), CONVERT(DATETIME, t1.InvoiceDate, 103), 105), 
						DATEADD (mi, 330, GETDATE()), @MobileAppId from #TBL_MOBILE_APP_Purchases t1
						Where Not Exists(Select 1 From TBL_MOBILE_APP_PURCHASES t2 Where t2.Gstin  = t1.Gstin and t2.ReferenceNo = t1.ReferenceNo
						 and t2.InvoiceDate = CONVERT(VARCHAR(50), CONVERT(DATETIME, t1.InvoiceDate, 103), 105))

					Update #TBL_MOBILE_APP_PURCHASES
						SET #TBL_MOBILE_APP_PURCHASES.PurchasesId = t2.PurchasesId 
						FROM #TBL_MOBILE_APP_PURCHASES t1,
						TBL_MOBILE_APP_PURCHASES t2 
						WHERE CONVERT(VARCHAR(50), CONVERT(DATETIME, t1.InvoiceDate, 103), 105) = t2.InvoiceDate 

					if exists (Select 1 FROM #TBL_MOBILE_APP_PURCHASES t1,
						TBL_MOBILE_APP_PURCHASES t2,
						TBL_MOBILE_APP_PURCHASES_ITMS t3
					...
					Insert TBL_MOBILE_APP_PURCHASES_ITMS( PurchasesId, ItemId, ItemShortName, ItemQty, Amount) 
							Select	distinct PurchasesId, ItemId, UPPER(ItemShortName), ItemQty, Amount
					From #TBL_MOBILE_APP_PURCHASES t1
					--Where Not Exists ( SELECT 1 FROM TBL_MOBILE_APP_PURCHASES_ITMS t2
					--				   Where t2.PurchasesId = t1.PurchasesId
					--				   And t2.ItemId = t1.ItemId
					--				   And t2.ItemShortName = t1.ItemShortName
					--				   And t2.ItemQty = t1.ItemQty
	...
	BEGIN
		...
		(
			...
			ItemQty varchar(50),
			Amount varchar(50),
			PurchasesId varchar(10), 
			PurchasesSubId varchar(10),
		...
		Begin
			...
			Select 
				...
				'' as PurchasesId,'' as PurchasesSubId,'' as errormessage,'' as errorcode
		...
		Begin
			Insert TBL_MOBILE_APP_PURCHASES	(Gstin, ReferenceNo, InvoiceDate, createddate, MobileAppId) 
				...
				Update #MobileAppRecordFldValues
					SET #MobileAppRecordFldValues.PurchasesId = t2.PurchasesId 
