...
Description : Procedure to Insert the External GSTR1 AT Records to the corresponding Staging
				Area tables
...
09/07/2017	Seshadri	Fixed AT Push Error 
...
exec usp_Push_GSTR1AT_EXT_SA  'POS','POSDEV1', '27AHQPA7588L1ZJ'

 */

CREATE PROCEDURE [usp_Push_GSTR1AT_EXT_SA]
...
Begin
	...
	SET #TBL_EXT_GSTR1_AT.gstr1id = t2.gstr1id 
	FROM #TBL_EXT_GSTR1_AT t1,
			TBL_GSTR1 t2 
	...
	-- Insert Records into Table TBL_GSTR1_AT

	Insert TBL_GSTR1_AT(gstr1id,pos,sply_ty,gstinid,
						custid,createdby,createddate) 
	Select	distinct gstr1Id , pos, sply_ty,gstinid,
					@Custid,@CreatedBy,GetDate()
	From #TBL_EXT_GSTR1_AT t1
	Where Not Exists ( SELECT 1 FROM TBL_GSTR1_AT t2
	...
	Update #TBL_EXT_GSTR1_AT
	SET #TBL_EXT_GSTR1_AT.atid = t2.atid 
	FROM #TBL_EXT_GSTR1_AT t1,
			TBL_GSTR1_AT t2 
	...
	From #TBL_EXT_GSTR1_AT t1
	Where	Not Exists ( SELECT 1 FROM TBL_GSTR1_AT_ITMS t2
