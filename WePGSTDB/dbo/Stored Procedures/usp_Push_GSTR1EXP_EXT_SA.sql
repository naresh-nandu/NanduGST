...
Description : Procedure to Insert the External GSTR1 EXP Records to the corresponding Staging
				Area tables
...
23/02/2018 	Raja  		Added csamt for Version 1.1  
...
exec usp_Push_GSTR1EXP_EXT_SA  'POS','POSDEV1', '27AHQPA7588L1ZJ'

 */

CREATE PROCEDURE [usp_Push_GSTR1EXP_EXT_SA]
...
Begin
	...
	(
		...
		Group By gstin,fp,gt,cur_gt,exp_typ,inum,idt,val,sbpcode,sbnum,sbdt,rt
	)t1
	Cross Apply 
	(
	...
	SET #TBL_EXT_GSTR1_EXP_INV.gstr1id = t2.gstr1id 
	FROM #TBL_EXT_GSTR1_EXP_INV t1,
			TBL_GSTR1 t2 
	...
	-- Insert Records into Table TBL_GSTR1_EXP

	Insert TBL_GSTR1_EXP(gstr1id,ex_tp,gstinId) 
	Select	distinct gstr1Id,exp_typ,gstinId
	From #TBL_EXT_GSTR1_EXP_INV t1
	Where Not Exists ( SELECT 1 FROM TBL_GSTR1_EXP t2 where t2.gstr1id = t1.gstr1id and t2.ex_tp = t1.exp_typ) 

	Update #TBL_EXT_GSTR1_EXP_INV 
	SET #TBL_EXT_GSTR1_EXP_INV.expid = t2.expid
	FROM #TBL_EXT_GSTR1_EXP_INV t1,
			TBL_GSTR1_EXP t2 
	WHERE t1.gstr1id = t2.gstr1id 
	And t1.exp_typ = t2.ex_tp 

	-- Insert Records into Table TBL_GSTR1_EXP_INV

	Insert TBL_GSTR1_EXP_INV
	(expid,inum,idt,val,sbpcode,sbnum,sbdt,gstinId,gstr1id,
		custid,createdby,createddate)
	Select	distinct t1.expid,t1.inum,t1.idt,t1.val,
			t1.sbpcode,t1.sbnum,t1.sbdt,t1.gstinId,t1.gstr1id,
			@Custid,@CreatedBy,GetDate()
	From #TBL_EXT_GSTR1_EXP_INV t1
	Where	Not Exists ( SELECT 1 FROM TBL_GSTR1_EXP_INV t2
	...
	Update #TBL_EXT_GSTR1_EXP_INV 
	SET #TBL_EXT_GSTR1_EXP_INV.invid = t2.invid 
	FROM #TBL_EXT_GSTR1_EXP_INV t1,
