...
Description : Procedure to Compute Checksum for GSTR1 B2CL Invoices
...
exec usp_Compute_Chksum_GSTR1_B2CL '10BNAPG9890G5ZK','062017'
 */

CREATE PROCEDURE [usp_Compute_Chksum_GSTR1_B2CL] 
	@Gstin varchar(15),
	@Fp varchar(10),
...
Begin
	...
	And t3.b2clid = t2.b2clid
	And t3.invid In (Select Value From @TBL_Values)
	And t3.flag = @Flag
	And t4.invid = t3.invid
	And t5.itmsid = t4.itmsid

	if Exists(Select 1 From #TBL_GSTR1_B2CL )
	Begin

		Declare InvCur Insensitive Cursor for
		Select Distinct invid,idt,inum,val   
		From #TBL_GSTR1_B2CL
		For Read Only

		Open InvCur
		Fetch InvCur Into @InvId,@Idt,@Inum,@Val  
		...
		Begin
			...
			Set @StrBuf = @StrBuf + 'val=' + Convert(Varchar,IsNull(@Val,0)) 

			Set @ChkSum = Convert(UniqueIdentifier,Hashbytes('SHA2_256',@StrBuf))

			Update #TBL_GSTR1_B2CL
			Set chksum = @ChkSum
			Where invid = @InvId
		...
		SET TBL_GSTR1_B2CL_INV.chksum = t2.chksum 
