...
Description : Procedure to Insert the External GSTR1 TXPA Records to the corresponding Staging
				Area tables
...
exec usp_Push_GSTR1TXPA_EXT_SA  'POS','POSDEV1', '27AHQPA7588L1ZJ'

 */

CREATE PROCEDURE [usp_Push_GSTR1TXPA_EXT_SA]
...
Begin
	...
	SET #TBL_EXT_GSTR1_TXPA.gstr1id = t2.gstr1id 
	FROM #TBL_EXT_GSTR1_TXPA t1,
			TBL_GSTR1 t2 
	...
	-- Insert Records into Table TBL_GSTR1_TXPA

	Insert TBL_GSTR1_TXPA(gstr1id,pos,sply_ty,gstinId,
	...
	Select	distinct gstr1Id , pos, sply_ty,gstinId,
					...
					omon,diff_percent
	From #TBL_EXT_GSTR1_TXPA t1
	Where Not Exists ( SELECT 1 FROM TBL_GSTR1_TXPA t2 
	...
	Update #TBL_EXT_GSTR1_TXPA
	SET #TBL_EXT_GSTR1_TXPA.txpaid = t2.txpaid 
	FROM #TBL_EXT_GSTR1_TXPA t1,
			TBL_GSTR1_TXPA t2 
	...
	-- Insert Records into Table TBL_GSTR1_TXPA_ITMS

	Insert TBL_GSTR1_TXPA_ITMS
	(txpaid,rt,
		ad_amt,iamt,camt,samt,csamt,gstinId,gstr1id)
	Select	distinct t1.txpaid,t1.rt,
			t1.ad_amt,t1.iamt,t1.camt,t1.samt,t1.csamt,t1.gstinId,t1.gstr1id
	From #TBL_EXT_GSTR1_TXPA t1
	Where	Not Exists ( SELECT 1 FROM TBL_GSTR1_TXPA_ITMS t2
						Where t2.txpaid = t1.txpaid 
							And t2.rt = t1.rt
						)		 

	If Exists(Select 1 From #TBL_EXT_GSTR1_TXPA)
	Begin
		Update TBL_EXT_GSTR1_TXPA 
		SET rowstatus = 0 
		Where (Ltrim(Rtrim(IsNull(@Gstin,''))) = '' or gstin = @Gstin)
