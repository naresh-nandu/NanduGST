...
Description : Procedure to Insert the GSTR1 TXP JSON Records to the corresponding external tables
...
exec usp_Insert_JSON_GSTR1TXP_EXT  'POS',
 */

CREATE PROCEDURE [usp_Insert_JSON_GSTR1TXP_EXT_TALLY]
...
Begin
	...
	Select @TotalRecordsCount = count(*) from #TBL_EXT_GSTR1_TXP
	...
	Begin
		...
		Update #TBL_EXT_GSTR1_TXP -- Introduced due to Excel Format Issue
		...
		Update #TBL_EXT_GSTR1_TXP 
		Set ErrorCode = -9,
			ErrorMessage = dbo.udf_ValidateGstAmount('TXP',Gstin,'','',Pos,Rt,'',Iamt,CAmt,Samt,Csamt,Ad_Amt,'','','','','') 
	...
	Begin
		...
		rt as [Rate],	
		ad_amt as [Gross Advance Adjusted],	
		csamt as [Cess Amount],
		ErrorCode,
		ErrorMessage
		From #TBL_EXT_GSTR1_TXP  Where IsNull(ErrorCode,0) <> 0
	End
	Else
	Begin
	Select 
