...
Description : Procedure to Insert the GSTR6 B2B JSON Records to the corresponding external tables
...
exec usp_Insert_JSON_GSTR6B2B_EXT  'POS',
 */

CREATE PROCEDURE [dbo].[usp_Insert_JSON_GSTR6B2B_EXT]
...
Begin
	...
	Select @TotalRecordsCount = count(*) from #TBL_EXT_GSTR6_B2B_INV
	...
	Begin
		...
		Update #TBL_EXT_GSTR6_B2B_INV -- Introduced due to Excel Format Issue
	...
	Select @ErrorRecords = (Select * From #TBL_EXT_GSTR6_B2B_INV
							Where IsNull(ErrorCode,0) <> 0
							FOR JSON AUTO)
