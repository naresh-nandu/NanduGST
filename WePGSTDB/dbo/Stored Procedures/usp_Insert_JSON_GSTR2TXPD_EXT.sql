...
Description : Procedure to Insert the GSTR2 TXPD JSON Records to the corresponding external tables
...
exec usp_Insert_JSON_GSTR2TXPD_EXT  'POS',
 */

CREATE PROCEDURE [usp_Insert_JSON_GSTR2TXPD_EXT]
...
Begin
	...
	Select @TotalRecordsCount = count(*) from #TBL_EXT_GSTR2_TXPD
	...
	Begin
		...
		Update #TBL_EXT_GSTR2_TXPD -- Introduced due to Excel Format Issue
		...
		Update #TBL_EXT_GSTR2_TXPD 
		Set ErrorCode = -9,
			ErrorMessage = dbo.udf_ValidateGstItcAmount('TXP',Gstin,'',Pos,Sply_Ty,Rt,'',Iamt,CAmt,Samt,Csamt,'','','','',
								Adamt,'','','','','') 
	...
	Select @ErrorRecords = (Select * From #TBL_EXT_GSTR2_TXPD
							Where IsNull(ErrorCode,0) <> 0
							FOR JSON AUTO)
