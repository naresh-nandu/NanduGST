...
exec usp_Insert_JSON_Mobile_App_Purchases_Bill  '33GSPTN0802G1ZL','WEP001','{
...
CREATE PROCEDURE [dbo].[usp_Insert_JSON_Mobile_App_Purchases_Bill]
...
Begin
	...
	BEGIN
		...
		Select	space(50) as PurchasesBillId,
				space(50) as PurchasesBillSubId,
				@Gstin as Gstin,
				...
				Into #TBL_MOBILE_APP_PURCHASES_BILL
		From OPENJSON(@RecordContents) 	
		WITH
		(
			PurchasesBillData nvarchar(max) as JSON
		) as PurchasesBill
		Cross Apply OPENJSON(PurchasesBill.PurchasesBillData)
		WITH
		(		
		...
		--Select * from #TBL_MOBILE_APP_Purchases
		Select @TotalRecordsCount = count(*) from #TBL_MOBILE_APP_PURCHASES_BILL

			
		Begin Try
			Begin
				Insert TBL_MOBILE_APP_PURCHASES_BILL(Gstin, ReferenceNo, InvoiceDate, createddate, MobileAppId) 
					Select Distinct Gstin, ReferenceNo, CONVERT(VARCHAR(50), CONVERT(DATETIME, t1.InvoiceDate, 103), 105), 
						DATEADD (mi, 330, GETDATE()), @MobileAppId from #TBL_MOBILE_APP_Purchases_BILL t1
						Where Not Exists(Select 1 From TBL_MOBILE_APP_PURCHASES_BILL t2 Where t2.Gstin  = t1.Gstin and t2.ReferenceNo = t1.ReferenceNo
						 and t2.InvoiceDate = CONVERT(VARCHAR(50), CONVERT(DATETIME, t1.InvoiceDate, 103), 105))

					Update #TBL_MOBILE_APP_PURCHASES_BILL
						SET #TBL_MOBILE_APP_PURCHASES_BILL.PurchasesBillId = t2.PurchasesBillId 
						FROM #TBL_MOBILE_APP_PURCHASES_BILL t1,
						TBL_MOBILE_APP_PURCHASES_BILL t2 
						WHERE CONVERT(VARCHAR(50), CONVERT(DATETIME, t1.InvoiceDate, 103), 105) = t2.InvoiceDate 

					if exists (Select 1 FROM #TBL_MOBILE_APP_PURCHASES_BILL t1,
						TBL_MOBILE_APP_PURCHASES_BILL t2,
						TBL_MOBILE_APP_PURCHASES_BILL_DET t3
					...
					Insert TBL_MOBILE_APP_PURCHASES_BILL_DET( PurchasesBillId, InvoiceNo, TotalItems, BillAmount, SupplierId, SupplierName) 
							Select	distinct PurchasesBillId, InvoiceNo, TotalItems, BillAmount, SupplierId, SupplierName
					From #TBL_MOBILE_APP_PURCHASES_BILL t1
					--Where Not Exists ( SELECT 1 FROM TBL_MOBILE_APP_PURCHASES_BILL_DET t2
			...
			Select * from #TBL_MOBILE_APP_PURCHASES_BILL

			Select @ProcessedRecordsCount = Count(*)
			From #TBL_MOBILE_APP_PURCHASES_BILL
			Where IsNull(ErrorCode,0) = 0
	...
	BEGIN
		...
		(
			...
			PurchasesBillId varchar(10),
			PurchasesBillSubId varchar(10),
		...
		Begin
			...
			Select 
				...
				dbo.udf_SplitGSTRRecordData(RecordContents, 9, @Delimiter) as SupplierId,
				dbo.udf_SplitGSTRRecordData(RecordContents, 10, @Delimiter) as SupplierName,
				'' as PurchasesBillId,'' as PurchasesBillSubId,'' as errormessage,'' as errorcode
		...
		Begin
			Insert TBL_MOBILE_APP_PURCHASES_BILL(Gstin, ReferenceNo, InvoiceDate, createddate, MobileAppId) 
				...
				Update #MobileAppRecordFldValues
					SET #MobileAppRecordFldValues.PurchasesBillId = t2.PurchasesBillId 
