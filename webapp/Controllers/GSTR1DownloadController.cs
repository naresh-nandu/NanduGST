...
namespace SmartAdminMvc.Controllers
{
    ...
    public class Gstr1DownloadController : Controller
    {
        ...
        public ActionResult Index(int page = 0, string sort = null)
        {
            ...
            try
            {
                if (sort != null || page != 0)
                {
                    ...
                    SelectList Actionlst = new SelectList(db.TBL_GSTR_ACTION_TYPE.Where(o => o.GSTRType == "GSTR1D"), "ActionName", "ActionName", Action);
        ...
        public ActionResult Index(FormCollection frm, string Download, string Export, string Command, string OTPSubmit, string command1)
        {
            ...
            string strGSTIN = "", strAction = "", strPeriod = "", strAUTHGSTIN = "", strRefId = "", strCTIN = "", strStateCode = "", strToken = "", strfromPeriod = "", strtoPeriod = "", strcommand = "", fp = "", gstin = "";
            ...
            try
            {
                ...
                if (!string.IsNullOrEmpty(Download))
                {
                    ...
                    SelectList Actionlst = new SelectList(db.TBL_GSTR_ACTION_TYPE.Where(o => o.GSTRType == "GSTR1D"), "ActionName", "ActionName", strAction);
                    ViewBag.ActionList = Actionlst;
                                    ...
                                    if (to_year == from_year)
                                    {
                                        ...
                                        for (int j = 0; j <= months; j++)
                                        {
                                            ...
                                            if (strAction == "ALL")
                                            {
                                                ...
                                                SelectValues.Add("B2CL");
                                                SelectValues.Add("B2CS");
                                                SelectValues.Add("CDNR");
                                                SelectValues.Add("CDNUR");
                                                SelectValues.Add("EXP");
                                                SelectValues.Add("NIL");
                                                SelectValues.Add("TXP");
                                                SelectValues.Add("AT");
                                                SelectValues.Add("HSNSUM");
                                                SelectValues.Add("DOCISS");
                                                SelectValues.Add("B2BA");
                                                SelectValues.Add("B2CLA");
                                                SelectValues.Add("B2CSA");
                                                SelectValues.Add("CDNRA");
                                                SelectValues.Add("CDNURA");
                                                SelectValues.Add("EXPA");
                                                SelectValues.Add("TXPA");
                                                SelectValues.Add("ATA");
                                                for (int J = 0; J < SelectValues.Count(); J++)
                                                {
                                                    Thread.Sleep(5000);
                                                    Task.Factory.StartNew(() => GSP_getGSTR1.SendRequestVoid(mySelectValues[i], strPeriod, SelectValues[J], strCTIN, strStateCode, strRefId, strToken, strfromPeriod, strtoPeriod, Session["User_ID"].ToString(), Session["Cust_ID"].ToString(), Session["UserName"].ToString(), "")
                                                    );
                                                    Thread.Sleep(3000);
                                            ...
                                            {
                                                Thread.Sleep(5000);
                                                Task.Factory.StartNew(() => GSP_getGSTR1.SendRequestVoid(mySelectValues[i], strPeriod, strAction, strCTIN, strStateCode, strRefId, strToken, strfromPeriod, strtoPeriod, Session["User_ID"].ToString(), Session["Cust_ID"].ToString(), Session["UserName"].ToString(), "")
                                                );
                                                Thread.Sleep(3000);
                                            }
                                        }
                    ...
                    {
                        string DownloadRes = GSP_getGSTR1.SendRequest(strGSTIN, strPeriod, strAction, strCTIN, strStateCode, strRefId, strToken, strfromPeriod, strtoPeriod, Session["User_ID"].ToString(), Session["Cust_ID"].ToString(), Session["UserName"].ToString(), "");
                        ...
                        if (to_year == from_year)
                        {
                            ...
                            for (int i = 0; i <= months; i++)
                            {
                                ...
                                if (strAction == "ALL")
                                {
                                    ...
                                    {
                                        Thread.Sleep(5000);
                                        Task.Factory.StartNew(() => GSP_getGSTR1.SendRequestVoid(strGSTIN, strPeriod, SelectValues[J], strCTIN, strStateCode, strRefId, strToken, strfromPeriod, strtoPeriod, Session["User_ID"].ToString(), Session["Cust_ID"].ToString(), Session["UserName"].ToString(), "")
                                        );
                ...
                else if (!string.IsNullOrEmpty(Export))
                {
                    ...
                    Response.AddHeader("content-disposition", "attachment; filename=GSTR1_Download_" + strGSTIN + "_" + strPeriod + ".xls");
                ...
                ViewBag.TitleHeaders = "GSTR-1 DOWNLOAD ";
                ViewBag.GSTINNoList = LoadDropDowns.Exist_GSTIN_No(iUserId, iCustId, Convert.ToString(strGSTIN), Session["Role_Name"].ToString());
                ViewBag.CTINNoList = LoadDropDowns.Exist_CustomerGSTIN(iCustId, strCTIN);
                SelectList Actionlst1 = new SelectList(db.TBL_GSTR_ACTION_TYPE.Where(o => o.GSTRType == "GSTR1D"), "ActionName", "ActionName", strAction);
                ViewBag.ActionList = Actionlst1;


                #region "LOAD DATA BASED ON ACTION"
                if (strAction == "B2B" || strAction == "B2CL" || strAction == "B2CS" || strAction == "CDNR" || strAction == "CDNUR" || strAction == "EXP" || strAction == "AT" || strAction == "TXP" || strAction == "B2BA" || strAction == "B2CLA" || strAction == "B2CSA" || strAction == "CDNRA" || strAction == "CDNURA" || strAction == "EXPA" || strAction == "ATA" || strAction == "TXPA" || strAction == "HSNSUM" || strAction == "NIL" || strAction == "DOCISS" || strAction == "ALL" || strAction== "FILEDET")
                {
                    if (!string.IsNullOrEmpty(strPeriod) && !string.IsNullOrEmpty(strfromPeriod) && !string.IsNullOrEmpty(strtoPeriod))
                    {
                        
                        DataSet GSTR2ASummary = new Gstr1Download(iCustId, iUserId).Retrieve_GSTR1_Download_EXT_SA_Bulk_Summary(strAction, strGSTIN, strfromPeriod, strtoPeriod, "");
                        TempData["GSTR1Summary"] = GSTR2ASummary;
            ...
            using (SqlConnection conn = new SqlConnection(ConfigurationManager.AppSettings["ConnectionString"]))
            {
                try
                {
                    ...
                    using (SqlCommand sqlcmd = new SqlCommand())
                    {
                        ...
                        sqlcmd.CommandText = "Select TOP 1 * from TBL_GSTR1_SAVE_RETSTATUS where gstin = @GSTINNo and fp = @Period and referenceno = @ReferenceId order by 1 desc";
