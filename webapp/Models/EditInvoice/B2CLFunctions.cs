...
{
    public class B2ClFunctions
    {
        readonly SqlConnection con = new SqlConnection(ConfigurationManager.AppSettings["ConnectionString"]);
        public int B2CLItemUpdate(int invid, decimal Rate, decimal TaxVal, decimal iamt, decimal camt, decimal samt, decimal csamt, string hsncode, string itemdesc, decimal qty, decimal unitprice, decimal discount, string uqc, int Createdby)
        {
            int outputparam;
            try
            {
                ...
                SqlCommand cmd = new SqlCommand("usp_Update_OUTWARD_GSTR1_B2CL_Items", con);
        ...
        public int B2CLItemDelete(int B2CLid)
        {
            int outputparam;
            try
            {
                ...
                SqlCommand cmd = new SqlCommand("usp_Delete_OUTWARD_GSTR1_B2CL_Items", con);
                cmd.CommandType = CommandType.StoredProcedure;
                cmd.CommandTimeout = 0;
