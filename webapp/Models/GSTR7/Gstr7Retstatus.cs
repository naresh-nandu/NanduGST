...
{
    public partial class Gstr7Retstatus
    {
        static SqlConnection con = new SqlConnection(ConfigurationManager.AppSettings["ConnectionString"]);
                

        public static void Update_GSTR7_RetStatus(string GSTINNo, string Period, string RefNo, string Action, string RefIds, string Status, string error_msg, string CustId, string UserId)
        {
            con.Open();
            ...
            if (Status == "1")
            {
                Common.Functions.InsertIntoTable("TBL_GSTR7_SAVE_RETSTATUS", cmd, con);
            }
            else
            {
                Common.Functions.UpdateTable("TBL_GSTR7_SAVE_RETSTATUS", "referenceno", RefNo, cmd, con);
            }
            con.Close();
        }
    }
