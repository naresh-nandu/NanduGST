...
{
    public class SendGSTR1AFileDataModel
    {
        ...
        private static string GSTR1AResponse = "";
        ...
        public string SendRequest(string JsonData, string GSTINNo, string Period, string SessionUserId, string SessionCustId, string SessionUsername)
        {
            try
            {
                ...
                GSTR1AFileParameters objGSTR1AFileParams = new GSTR1AFileParameters();
                objGSTR1AFileParams.data = encryptedpayload;
                objGSTR1AFileParams.hmac = encryptedhmac;
                objGSTR1AFileParams.action = "RETSUBMIT";

                string tempjsondata = new JavaScriptSerializer().Serialize(objGSTR1AFileParams);

                var httpWebRequest = (HttpWebRequest)WebRequest.Create(ConfigurationManager.AppSettings["GST_GSTR1A"]);
                ...
                using (var streamWriter = new StreamWriter(httpWebRequest.GetRequestStream()))
                {
                    ...
                    using (var streamReader = new StreamReader(httpResponse.GetResponseStream()))
                    {
                        ...
                        GSTR1AResponse = result.ToString();
                        //File.WriteAllText(@"E:\GSTR1.json", GSTR1Response);
                    }
                }

                var data = (JObject)JsonConvert.DeserializeObject(GSTR1AResponse);
                ...
                if (status_response == "1")
                {
                    ...
                    cmd.Parameters.Add("@GSTRName", SqlDbType.VarChar).Value = "GSTR1A";
                    ...
                    cmd.Parameters.Add("@Audit_Message", SqlDbType.VarChar).Value = "GSTR1A Filed";
                    ...
                    Common.Notification.SendSMS(strMobileNo, string.Format("Congratulations!! Your GSTR - 1A is Filed Successfully!!!"));
                    return "GSTR1A Filed Successfully... Status - " + laststatus_response + " and Ack No. - " + Acknum_response;

                }
                else
                {
                    var root = JsonConvert.DeserializeObject<Common.ErrorMsg>(GSTR1AResponse);
    ...
    public class GSTR1AFileParameters
    {
