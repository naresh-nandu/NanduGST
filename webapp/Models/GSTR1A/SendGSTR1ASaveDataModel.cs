...
{
    public class SendGSTR1ASaveDataModel
    {
        ...
        public string SendRequest(string JsonData, string GSTINNo, string Period, string SessionUserId, string SessionCustId, string SessionUsername)
        {
            try
            {
                ...
                GSTR1ASaveParameters objGSTR1ASaveParams = new GSTR1ASaveParameters();
                objGSTR1ASaveParams.data = encryptedpayload;
                objGSTR1ASaveParams.hmac = encryptedhmac;
                objGSTR1ASaveParams.action = "RETSAVE";

                string tempjsondata = new JavaScriptSerializer().Serialize(objGSTR1ASaveParams);
                ...
                if (status_response == "1")
                {
                    ...
                    cmd.Parameters.Add("@Audit_Message", SqlDbType.VarChar).Value = "GSTR1A Saved";
                    cmd.Parameters.Add("@Audit_Exception", SqlDbType.VarChar).Value = "";
                    Common.Functions.InsertIntoTable("TBL_Audit_Log", cmd, con);

                    con.Close();
                    return "GSTR1A Saved Successfully... Trans Id - " + transid_response + " and Ref Id - " + refid_response;
                }
                else
                {
    ...
    public class GSTR1ASaveParameters
    {
