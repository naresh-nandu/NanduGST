        ...
        public string SendRequest(DataTable data)
        {
            try
            {
                ...
                if (data.Rows.Count > 0)
                {
                    ...
                    {
                        SqlDataAdapter adt = new SqlDataAdapter("Select * from TBL_BP_GSTR2_B2BA_INV where num = '" + data.Rows[i][4].ToString() + "' and dt = '" + data.Rows[i][4].ToString() + "'", con);
                        ...
                        if (dt.Rows.Count > 0)
                        {
                            ...
                            cmd.Parameters.Add("@ty", SqlDbType.VarChar).Value = data.Rows[i][7].ToString();
                            cmd.Parameters.Add("@hsn_sc", SqlDbType.VarChar).Value = data.Rows[i][8].ToString();
                            cmd.Parameters.Add("@txval", SqlDbType.Decimal).Value = data.Rows[i][9].ToString();
                            ...
                            BPInv_Itms_Response = Common.Functions.InsertIntoTable("TBL_BP_GSTR2_B2BA_INV_ITMS", cmd, con);
                        }
                        else
                        {
                            cmd.Parameters.Clear();
                            ...
                            cmd.Parameters.Add("@onum", SqlDbType.VarChar).Value = data.Rows[i][2].ToString();
                            cmd.Parameters.Add("@odt", SqlDbType.VarChar).Value = data.Rows[i][3].ToString();
                            cmd.Parameters.Add("@num", SqlDbType.VarChar).Value = data.Rows[i][4].ToString();
                            cmd.Parameters.Add("@dt", SqlDbType.VarChar).Value = data.Rows[i][5].ToString();
                            cmd.Parameters.Add("@val", SqlDbType.Decimal).Value = data.Rows[i][6].ToString();
                            BPInv_Response = Common.Functions.InsertIntoTable("TBL_BP_GSTR2_B2BA_INV", cmd, con);
